"""Daglig aggregering – kors varje natt."""
...
def fetch_spot(date):
    ...
    log.info("Hamtar spotpriser: %s", url)
...
def process_day(date_str):
    ...
    if not raw_file.exists():
        log.warning("Ingen radata for %s", date_str)
        return None
    ...
    if len(records) < 100:
        log.warning("Bara %d pollningar for %s", len(records), date_str)
        return None

    log.info("Laste %d pollningar for %s", len(records), date_str)
    ...
    df["dt_h"] = df["dt_h"].fillna(df["dt_h"].median())
    ...
    try:
        ...
        log.error("Kunde inte hamta spotpriser: %s", e)
        return None

    df["quarter"] = df["ts"].apply(quarter_floor)
    df["spot"] = df["quarter"].map(spot)
    if df["spot"].isna().any():
        log.warning("Saknade spotpriser for %d rader", df["spot"].isna().sum())
    ...
    log.info("%s: Sol=%.2f kr, Bat=%.2f kr, TOTAL=%.2f kr", date_str, sol_kr, bat_kr, total)
    return result
